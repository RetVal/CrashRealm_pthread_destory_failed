...
@interface RxMultipleDelegateEnumerator ()
{
    NSUInteger numNodes;
...
static const void *__RxMultipleDelegateQueueIdentifyKey __unused = &__RxMultipleDelegateQueueIdentifyKey;
static const char *__RxMultipleDelegateQueueIdentifier = "com.Rx.multipleDelegateIdentifier";

@implementation RxMultipleDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    dispatch_queue_set_specific(delegateQueue, (__bridge const void *)self, (void *)__RxMultipleDelegateQueueIdentifier, nil);
    
    RxMultipleDelegateNode *node =
    [[RxMultipleDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    {
        RxMultipleDelegateNode *node = [delegateNodes objectAtIndex:(i-1)];
        ...
        if (delegate == nodeDelegate)
        {
            ...
            {
                // Recall that this node may be retained by a RxMultipleDelegateEnumerator.
...
{
    for (RxMultipleDelegateNode *node in delegateNodes)
    {
        node.delegate = nil;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        node.unsafeDelegate = nil;
...
{
    return [[RxMultipleDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
- (void)forwardInvocation:(NSInvocation *)origInvocation
{
    ...
    for (RxMultipleDelegateNode *node in [delegateNodes copy])
    {
        id nodeDelegate = node.delegate;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        if (nodeDelegate == [NSNull null])
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        bool shouldCallDispatch = dispatch_queue_get_specific(dispatch_get_current_queue(), (__bridge const void *)self) != __RxMultipleDelegateQueueIdentifier;
...
@implementation RxMultipleDelegateNode
...
@implementation RxMultipleDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
    ...
    {
        RxMultipleDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
        currentNodeIndex++;
